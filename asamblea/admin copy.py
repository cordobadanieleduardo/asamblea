...
class CustomUserAdmin(BaseUserAdmin):
    model = Militante
    list_display =  BaseUserAdmin.list_display + ('username', 'email', 'first_name', 'last_name','sex','plancha' ,'is_staff', 'is_active','send_email','must_change_password','position','list','votos_emitidos',)  # Campos visibles en el listado
    ...
    fieldsets = (
        ...
        ('Información Personal', {'fields': ('first_name', 'last_name','sex',)}),
    ...
    add_fieldsets = (
        (None, {
            'classes': ('wide',),
            'fields': ('username', 'email', 'password1', 'password2', 'first_name', 'last_name', 'sex', 'is_staff', 'is_active','plancha','send_email','must_change_password','position','list',)}
        ),
    )
